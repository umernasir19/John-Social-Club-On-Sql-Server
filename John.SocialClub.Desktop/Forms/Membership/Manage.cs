        ...
        private void dataGridViewMembers_SelectionChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dgv.SelectedRows.Count > 0)
                {
                    ...
                    dt2DateOfBirth.Value = DateTime.Parse((dataRow["DOb"].ToString()));
                    cmb2Occupation.SelectedItem = (Occupation)Enum.Parse(typeof(Occupation),dataRow["Occupation"].ToString(),true);
                    cmb2MaritalStatus.SelectedItem = (MaritalStatus)Enum.Parse(typeof(MaritalStatus),dataRow["MaritalStatus"].ToString(),true);
                    cmb2HealthStatus.SelectedItem = (HealthStatus)Enum.Parse(typeof(HealthStatus),dataRow["Health"].ToString(),true);
                    txt2Salary.Text = dataRow["Salary"].ToString() == "0.0000" ? string.Empty : dataRow["Salary"].ToString();
                    txt2NoOfChildren.Text = dataRow["No_Of_Children"].ToString();
                }
            }
            catch (Exception ex)
            {
